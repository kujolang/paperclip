...
export const pluginConfigSchema = z.object({
  ...
  }).strict().default({ binary: null, allowSystemPathFallback: true }),
  features: z.object({
    review: z.boolean().default(true),
    failureEvidence: z.boolean().default(true),
    context: z.boolean().default(true),
    verification: z.boolean().default(false),
  }).strict().default({ review: true, failureEvidence: true, context: true, verification: false }),
  limits: z.object({
    timeoutMs: z.number().int().min(1_000).max(27_000).default(DEFAULT_LIMITS.timeoutMs),
...
export const instanceConfigJsonSchema = {
  ...
  properties: {
    ...
    limits: {
      ...
      additionalProperties: false,
      properties: {
        timeoutMs: { type: "integer", minimum: 1000, maximum: 27000, default: DEFAULT_LIMITS.timeoutMs },
